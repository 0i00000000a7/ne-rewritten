...
// 数组位置推算
//
// children 中 index 从左到右严格递增。
// 当前位置 = node.index - first_child.index。
// ---------------------------------------------------------------------------
...
export function next_sibling<T>(
    node: TreeNode<T>,
    skip = 0,
): TreeNode<T> | undefined {
    const parent = node.parent
